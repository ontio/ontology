...
func (this *BlockSyncMgr) syncHeader() {
	...
	reqNode := this.getNextNode(curBlockHeight)
	if reqNode == nil {
		return
	}
	this.addFlightHeader(reqNode.id, curHeaderHeight+1)
	message.SendMsgSyncHeaders(reqNode)
	log.Infof("SendMsgSyncHeaders Request Height:%d", curHeaderHeight+1)
}
...
func (this *BlockSyncMgr) syncBlock() {
	...
	availCount := uint32(SYNC_MAX_FLIGHT_BLOCK_SIZE - this.getFlightBlockCount())
	if availCount <= 0 {
		return
	}
	curBlockHeight, err := actor.GetCurrentBlockHeight()
	...
	cacheCap := uint32(SYNC_MAX_BLOCK_CACHE_SIZE - this.getBlockCacheSize())
	if count > cacheCap {
		count = cacheCap
	}
...
func (this *BlockSyncMgr) checkBlockCache() {
	...
	for _, block := range this.blockCacheList {
		...
		if block.Header.Height <= curBlockHeight {
			clearList = append(clearList, block)
