...
func (self *ChainManager) buildShardConfig(shardID common.ShardID, shardState *shardstates.ShardState) (*config.OntologyConfig, error) {
	...
	if shardConfig.Genesis.ConsensusType == config.CONSENSUS_TYPE_SOLO {
		...
		shardView, err := xshard.GetShardView(ledger.GetShardLedger(common.NewShardIDUnchecked(config.DEFAULT_SHARD_ID)), writeSet, shardState.ShardID)
		if err != nil {
			return nil, fmt.Errorf("buildShardConfig GetShardView: failed, err: %s", err)
		}
		peerStakeInfo, err := xshard.GetShardPeerStakeInfo(ledger.GetShardLedger(common.NewShardIDUnchecked(config.DEFAULT_SHARD_ID)), writeSet, shardState.ShardID, shardView.View)
		if err != nil {
