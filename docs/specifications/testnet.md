...
We can perform a quick deployment by modifying the default configuration file `config.json`, click [here](config.md) to read 
...
We can perform a quick deployment by modifying the default configuration file `config-dbft.json`, click [here](config.md) to read 
 instruction of config file.
