...
func AcceptAdmin(native *native.NativeService) ([]byte, error) {
	...
	if err := destinationAdmin.Deserialize(bytes.NewBuffer(native.Input)); err != nil {
		return utils.BYTE_FALSE, errors.NewErr("[AcceptAdmin]Deserialize Admins failed!")
	}
	if !native.ContextRef.CheckWitness(common.Address(*destinationAdmin)) {
		return utils.BYTE_FALSE, errors.NewErr("[AcceptAdmin]Authentication failed!")
	}
	contract := native.ContextRef.CurrentContext().ContractAddress
	...
	if err != nil || *transferAdmin != *destinationAdmin {
		return utils.BYTE_FALSE, fmt.Errorf("[AcceptAdmin] Destination account hasn't been approved: %v", err)
	}
...
func TransferAdmin(native *native.NativeService) ([]byte, error) {
	...
	if !native.ContextRef.CheckWitness(common.Address(*admin)) {
		return utils.BYTE_FALSE, errors.NewErr("[TransferAdmin]Authentication failed!")
	}
	destinationAdmin := new(Admin)
	if err := destinationAdmin.Deserialize(bytes.NewBuffer(native.Input)); err != nil {
		return utils.BYTE_FALSE, errors.NewErr("[TransferAdmin]Deserialize Admins failed!")
	}
...
func SetGlobalParam(native *native.NativeService) ([]byte, error) {
	...
	getAdmin(native, contract)
	if !native.ContextRef.CheckWitness(common.Address(*admin)) {
		return utils.BYTE_FALSE, errors.NewErr("[SetParam]Authentication failed!")
	}
	params := new(Params)
	if err := params.Deserialize(bytes.NewBuffer(native.Input)); err != nil {
		return utils.BYTE_FALSE, errors.NewErr("[SetParam]Deserialize failed!")
	}
...
func GetGlobalParam(native *native.NativeService) ([]byte, error) {
	...
	if err := paramNameList.Deserialize(bytes.NewBuffer(native.Input)); err != nil {
		return utils.BYTE_FALSE, errors.NewErr("[GetParam]Deserialize failed!")
	}
	params := new(Params)
	*params = make(map[string]string, 0)
	...
	if len(paramNotInCache) == 0 { // all request param exist in cache
		...
		if err != nil {
			return utils.BYTE_FALSE, errors.NewDetailErr(err, errors.ErrNoCode, "[GetParam] results to josn error!")
		}
		return result, nil
	}
	// read from db
	...
	if err != nil {
		return utils.BYTE_FALSE, errors.NewDetailErr(err, errors.ErrNoCode, "[GetParam] storage error!")
	}
	...
	for _, paramName := range paramNotInCache { // read param not in cache
		if value, ok := (*storageParams)[paramName]; ok {
			...
			return utils.BYTE_FALSE, errors.NewErr("[GetParam] param doesn't exist!")
		}
	}
	result, err := json.Marshal(params)
	if err != nil {
...
func CreateSnapshot(native *native.NativeService) ([]byte, error) {
	...
	getAdmin(native, contract)
	if !native.ContextRef.CheckWitness(common.Address(*admin)) {
		return utils.BYTE_FALSE, errors.NewErr("[CreateSnapshot]Authentication failed!")
	}
	// read prepare param
	prepareParam, err := getStorageParam(native, getParamKey(contract, PREPARE_VALUE))
	if err != nil {
		return utils.BYTE_FALSE, errors.NewDetailErr(err, errors.ErrNoCode, "[CreateSnapshot] storage error!")
	}
	if len(*prepareParam) == 0 {
		return utils.BYTE_FALSE, errors.NewErr("[CreateSnapshot] Prepare param doesn't exist!")
	}
