...
func isBalanceEnough(address common.Address, gas uint64) bool {
	balance, _, err := hComm.GetNativeTokenBalance(0, []common.Address{utils.OngContractAddress}, address, false)
	if err != nil {
...
func (ta *TxPoolService) handleTransaction(sender tc.SenderType, txn *tx.Transaction, txResultCh chan *tc.TxResult) {
	...
	if stateful.IsSenderLimited(txn.GetSignatureAddresses()) {
		log.Infof("transaction limited: %s, raw: %x", txn.Hash().ToHexString(), txn.Raw)
		replyTxResult(txResultCh, txn.Hash(), errors.ErrNoAccount, "limited")
	}
