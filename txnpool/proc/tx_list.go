...
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(uint64(tx.Nonce)) != nil
}
...
func (l *txList) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	...
	if old != nil {
		...
		a = a.Mul(a, big.NewInt(int64(old.GasPrice)))
		b := big.NewInt(100)
		threshold := a.Div(a, b)
...
func (l *txList) Filter(costLimit *big.Int, gasLimit uint64) (Transactions, Transactions) {
	...
	if l.strict {
		...
		for _, tx := range removed {
			if nonce := uint64(tx.Nonce); lowest > nonce {
				lowest = nonce
			}
		}
		invalids = l.txs.filter(func(tx *types.Transaction) bool { return uint64(tx.Nonce) > lowest })
	}
	l.txs.reheap()
...
func (l *txList) Remove(tx *types.Transaction) (bool, Transactions) {
	...
	if l.strict {
		return true, l.txs.Filter(func(tx *types.Transaction) bool { return uint64(tx.Nonce) > nonce })
	}
	return true, nil
}
