...
func DataReqHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	case common.BLOCK:
		...
		if msg == nil {
			...
			if db == nil {
				// the remote node send us wrong shard id
				return
			}
	...
	case common.TRANSACTION:
		...
		txn, err := db.GetTransaction(hash)
		if err != nil {
...
func InvHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	case common.BLOCK:
		...
		for _, id = range inv.P.Blk {
			...
			isContainBlock, err := db.IsContainBlock(id)
			if err != nil {
				log.Warn(err)
				return
			}
...
func GetHeadersFromHash(shardId common.ShardID, startHash common.Uint256, stopHash common.Uint256) ([]*types.RawHeader, error) {
	...
	curHeight := db.GetCurrentHeaderHeight()
	if startHash == common.UINT256_EMPTY {
		if stopHash == common.UINT256_EMPTY {
			...
			bkStop, err := db.GetRawHeaderByHash(stopHash)
		...
		bkStart, err := db.GetRawHeaderByHash(startHash)
	...
	var i uint32
	for i = 1; i <= count; i++ {
		hash := db.GetBlockHash(stopHeight + i)
		hd, err := db.GetRawHeaderByHash(hash)
		if err != nil {
