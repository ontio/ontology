...
func EmergencyRequestHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	log.Debug("RX emergency request message", data.Addr, data.Id)

	if pid != nil {
		msg := data.Payload.(*msgTypes.EmergencyReqMsg)
...
func EmergencyResponseHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	log.Debug("RX emergency response message", data.Addr, data.Id)

	if pid != nil {
		msg := data.Payload.(*msgTypes.EmergencyRspMsg)
		input := &msgCommon.EmergencyMsg{
