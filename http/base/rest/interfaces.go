...
func SendRawTransaction(cmd map[string]interface{}) map[string]interface{} {
	...
	if txn.ShardID == chainmgr.GetShardID() {
		...
		resp["Error"] = ontErrors.ErrInValidShard
		resp["Result"] = ontErrors.ErrInValidShard.Error()
		return resp
	}
...
func GetShardTxState(cmd map[string]interface{}) map[string]interface{} {
	...
	notifyId, _ := cmd["NotifyId"]
	var nid uint64
	var hasNotifyId bool
	if notifyId != "" {
		nidstr, nok := notifyId.(string)
		if !nok {
			return ResponsePack(berr.INVALID_PARAMS)
		}
	...
	value, err := bactor.GetShardTxState(txHash, uint32(nid), hasNotifyId)
	if err != nil {
		return ResponsePack(berr.INTERNAL_ERROR)
	}
