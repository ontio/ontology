...
import (
	...
	evmerrors "github.com/ontio/ontology/vm/evm/errors"
)
...
func (t *flatCallTracer) CaptureStart(env *evm.EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
...
func (t *flatCallTracer) CaptureState(pc uint64, op evm.OpCode, gas, cost uint64, scope *evm.ScopeContext, rData []byte, depth int, err error) {
	t.tracer.CaptureState(pc, op, gas, cost, scope, rData, depth, err)
}

// CaptureFault implements the EVMLogger interface to trace an execution fault.
func (t *flatCallTracer) CaptureFault(pc uint64, op evm.OpCode, gas, cost uint64, scope *evm.ScopeContext, depth int, err error) {
	t.tracer.CaptureFault(pc, op, gas, cost, scope, depth, err)
}

// CaptureEnter is called when EVM enters a new scope (via call, create or selfdestruct).
func (t *flatCallTracer) CaptureEnter(typ evm.OpCode, from common.Address, to common.Address, input []byte, gas uint64, value *big.Int) {
...
func (t *flatCallTracer) CaptureExit(output []byte, gasUsed uint64, err error) {
	...
	if typ == evm.CALL || typ == evm.STATICCALL {
...
func flatFromNested(input *callFrame, traceAddress []int, convertErrs bool, ctx *tracers.Context) (output []flatCallFrame, err error) {
	...
	case evm.CALL, evm.STATICCALL, evm.CALLCODE, evm.DELEGATECALL:
	...
	if input.Error != "" && input.Error != evmerrors.ErrExecutionReverted.Error() {
		frame.Result = nil
	}
...
func newFlatCreate(input *callFrame) *flatCallFrame {
	...
	return &flatCallFrame{
		Type: strings.ToLower(evm.CREATE.String()),
...
func newFlatCall(input *callFrame) *flatCallFrame {
	...
	return &flatCallFrame{
		Type: strings.ToLower(evm.CALL.String()),
		Action: flatCallAction{
			From:     &input.From,
			To:       input.To,
			Gas:      &input.Gas,
