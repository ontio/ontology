...
func (f *callFrame) processOutput(output []byte, err error) {
	...
	f.Error = err.Error()
	if f.Type == evm.CREATE || f.Type == evm.CREATE2 {
		f.To = nil
	}
	if !errors.Is(err, evmerrors.ErrExecutionReverted) || len(output) == 0 {
		return
	}
	f.Output = output
	if len(output) < 4 {
