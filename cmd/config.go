...
func setP2PNodeConfig(ctx *cli.Context, cfg *config.P2PNodeConfig) {
	...
	cfg.NodePortLegacy = ctx.Uint(utils.GetFlagName(utils.NodePortFlagLegacy))
	cfg.NodeConsensusPort = ctx.Uint(utils.GetFlagName(utils.ConsensusPortFlag))
	cfg.NodeConsensusPortLegacy = ctx.Uint(utils.GetFlagName(utils.ConsensusPortFlagLegacy))
